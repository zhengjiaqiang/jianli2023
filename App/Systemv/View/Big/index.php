...
<body class="diy-css list-fixed" id="layer-open-parent">
	<div class="list-siderbar">
		<div class="opr-btns">
			<if condition="$add">
				<a class="btn btn-primary layer-open-cool" data-w="600px" data-h="500px" title="添加"  shref="__CONTROLLER__/edit/id/{$id}"><i class="icon icon-plus"></i> 添加</a>
			</if>
            <if condition="$sort">
			...
			MCMS.table.Init(onePageRow,controller+'/index?id='+pid,[
					...
					{   "aTargets": [0],
						...
						"mRender": function (data, type, full) { 
							return MCMS.table.pattern(full.btime,'yyyy-MM-dd')+'至'+MCMS.table.pattern(full.etime,'yyyy-MM-dd');
						}	
					},{
						"aTargets": [5],
						"mRender": function (data, type, full) { 
							...
							if(up){
								_h+='<a class="green layer-open-cool"  data-w="550px" data-h="680px" title="编辑" shref="'+controller+'/edit?pid='+full.id+'&id='+pid+'"><i class="icon-pencil bigger-130"></i>  编辑</a>';
								_h+=' <a class="green"  role="event-btn" data-value="'+data+'" data-e="on"><i class="icon-off bigger-130"></i> '+(full.status==1 ? '关闭' : '开启')+'</a>';
							}
							if(del){
								_h+='<a class="red " title="删除" href="javascript:void(0);" data-id="'+full.id+'" data-stitle="确定删除当前大类？" role="delete"  data-loading-text="删除中..."><i class="icon-trash bigger-130"></i>  删除</a></div>';
							}
                          return _h;
                        }
					}
