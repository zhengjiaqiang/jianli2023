			...
			<div class="row">
				<div class="col-sm-6">
					...
					<else />
						...
						<div class="form-group txt-span">
							<label>登录密码 </label>
							<?php html::text('password',null,array('style'=>'width:200px;','placeholder'=>'登录密码')); ?><span style="color:red;font-size:10px;">*密码最低8位且包括一个大写、一个小写、一个符号</span>
						</div>
					</if>
					<div class="space-4"></div>
					<div class="form-group txt-span">
						...
						<if condition="empty($info)">
							<?php html::select('rid',$dlist,get_array_val($info,'rid'),array('style'=>'width:200px;'),null,'--请选择权限类型--');?>
						<else/><?php html::select('rid',$dlist,get_array_val($info,'rid'),array('style'=>'width:200px;'),null);?>
						</if>
		...
		<script>
			$(function(){
				...
				$("#edit-form-area").submit(function(){
					MCMS.commajaxfun(this);return false;
				});
