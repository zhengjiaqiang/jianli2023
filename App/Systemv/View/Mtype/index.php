...
<body class="diy-css list-fixed">
	<div class="list-siderbar">
		<div class="opr-btns">
			...
			<if condition="$delete">
				<button type="button" class="btn btn-primary" data-stitle="确定删除当前栏目类型?（<span style='color:red;font-size:12px'>删除之后当前类型下的所有栏目不可访问！</span>）" role-e="delete" data-loading-text="删除中..."><i class="icon icon-remove"></i> 删除已选项</button>
			</if>
		...
		<table id="main-table-frist" class="table table-striped table-bordered table-hover">
			<thead>
				<tr>
					...
					<th width="40">编号</th>
					<th class="left"  width="120">类型名称</th>
					<th class="left" width="120">显示路径</th>
                    <th width="80">后台路径</th>
			...
			MCMS.table.Init(onePageRow,controller+'/index?id='+pid,[
					...
					{   "aTargets": [0],
						...
						"mRender": function (data, type, full) {
                          var _h = '/'+full['controller']+'/'+full['view'];
                           return _h;
                          },
					},{   "aTargets": [4],
						"mRender": function (data, type, full) {
							//='/'+full['controller']+'/'+full['view']
                          var _h =(typeof(full.system) == 'object' || full.system =='') ? '/systemv/'+full['controller']+'/'+full['view'] : full.system;
                           return _h;
                          },
					},{
						"aTargets": [5],
						...
						"mRender": function (data, type, full) { 
							...
							if(del){
								var ti="（<span style='color:red;font-size:12px'>删除之后当前类型下的所有栏目不可访问！</span>）"
								_h+='<a class="red " title="删除" href="javascript:void(0);" data-id="'+full.id+'" role="delete"  data-stitle="确定删除当前栏目类型?'+ti+'"  data-loading-text="删除中..."><i class="icon-trash bigger-130"></i>  删除</a></div>';
							}
                          return _h;
                        }
					}
