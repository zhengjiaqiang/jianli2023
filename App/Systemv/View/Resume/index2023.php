...
<head>
	...
	<style type="text/css">
		...
		.data-remark{display: block;width:90px;text-align: center;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;}
	</style>
</head>
<body class="diy-css list-fixed" id="layer-open-parent">
	<div class="list-siderbar">
		<div class="opr-btns">
			...
			<elseif condition="$type eq 4"/><button type="button" class="btn btn-primary" data-stitle="修改面试时间"  role-e="noticedate"  data-loading-text="通知中..."><i class="icon icon-file"></i> 修改面试时间</button>
			...
			<div class="btn-group" >
				...
				<ul class="dropdown-menu" role="event-list"  style="min-height:50px;overflow-y:auto;">
					<li><a data-e="information" >导出基本信息</a></li>
					<li><a data-e="word">导出Word</a></li>
					<li><a data-e="wordlist">导出多个简历压缩包</a></li>
					<li><a data-e="enclosure">导出上传附件</a></li>
				</ul>
			</div>
            ...
            <div class="box-search">
				<div class="widget-main">
					<div class="row">
						...
						<div class="rowdiv">
							<label class="labelonly">手机号：</label>
							<div class="label-right">
								<input name="2ywi8z97l7" class="input-sm" type="text" id="mobile" placeholder="手机号">
							</div>
						</div>
						<div class="rowdiv">
					...
					<!-- 	<div class="rowdiv">
						...
						<div class="rowdiv">
							<label class="labelonly">在职毕业院校：</label>
							<div class="label-right">
								<input name="1wqxq7r9dpdyg" class="input-sm" type="text" id="major" placeholder="在职毕业院校">
							</div>
						</div>

						<div class="rowdiv">
							<label class="labelonly">全日制院校：</label>
							<div class="label-right">
								<input name="1xq7r9dpdyg" class="input-sm" type="text" id="major" placeholder="全日制院校">
							</div>
						</div>
						<div class="rowdiv">
						...
						<div class="rowdiv">
							...
							<div class="label-right">
								<select name="2izv9jv98uk">
									<option value=""></option>
									<option value="是">应届生</option>
									<option value="否">历届生</option>
								</select>
							</div>
						</div>
						<div class="rowdiv">
							<label class="labelonly">住院规培：</label>
							<div class="label-right">
								<select name="1etmnffrzs8">
									<option value=""></option>
		...
		<table id="main-table-frist" class="table table-striped table-bordered table-hover">
			<thead>
				<tr>
					...
					<th width="60">邮件</th>
					<th width="120" class="fc"><if condition="$type eq 4">面试时间<else/>投递时间</if></th>
					<th width="80" class="fc">状态</th>
					<th width="80" class="fc"><if condition="$type eq 4">评审结果<else/>备注</if></th>
					<th width="30">在职院校</th>
					<th width="30">全日制院校</th>
					<if condition="$type eq 2">
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			MCMS.table.Init(onePageRow,controller+'/index?id='+pid+"&type="+_type,[
					...
					__i("<if condition='$type eq 4'>date<else/>intimes</if>",null,false),
					__i("show",1,false),
					__i("id",1,false),
					__i("1wqxq7r9dpdyg",1,false),
					__i("1xq7r9dpdyg",1,false),
					<if condition="$type eq 2">
					__i("id",0,false),</if>
					__i("id",0,false),
					__i("id",0,false)
					...
					{   "aTargets": [0],
						...
						"aTargets": [15],
						"mRender": function (data, type, full) { 
							return  !CheckIsNullOrEmpty(data) ?  '无' :  (data == '其他' ? full['26sdjmrs8kmsz'] : data) ;
						}
					},{
						"aTargets": [16],
						"mRender": function (data, type, full) { 
							return  !CheckIsNullOrEmpty(data) ?  '无' :  (data == '其他' ? full['26jmrsdgsds8kmsz'] : data) ;
						}
					},{
						"aTargets": [14],
						"mRender": function (data, type, full) { 
							...
							<if condition="$type eq 4">
								return '<a class="layer-open-cool" data-w="500px" data-stitle="面试情况" data-h="400px" shref="/systemv/resume/situation/id/'+pid+'/pid/'+full.id+'"><span style="color:green;">'+full.passcount+'</span>通过<br/><span style="color:red;">'+full.nopasscount+'</span>未通过</a>';
							<else/>
								return '<a href="javascript:;" class="data-remark" data-id="'+data+'" title="'+full.remark+'" role="remark" >'+(CheckIsNullOrEmpty(full.remark)  ? full.remark : '点击备注')+'</a>';
							</if>
							// return '<a href="javascript:;" class="data-remark" data-id="'+data+'" title="'+full.remark+'" role="remark" >'+(CheckIsNullOrEmpty(full.remark)  ? full.remark : '点击备注')+'</a>';
						}
					},
					...
					{
						"aTargets": [<if condition="$type eq 2">18<else/>17</if>],
						"mRender": function (data, type, full) { 
							return '<input type="text" name="sort['+full.id+']" value="'+full.sort+'" style="width:70px;" class="input"/>';
						}
					},
					{
						"aTargets": [<if condition="$type eq 2">19<else/>18</if>],
						"mRender": function (data, type, full) { 
