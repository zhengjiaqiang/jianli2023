...
class EmailuserController extends BaseController{
	...
	public function operate(){
		...
		if($event == 'delete'){
			...
			if($rs === false) $this->error('当前邮箱删除失败，请刷新网络重试！');
			else{
				$this->addLog('delete','删除邮箱IDS：'.$ids);
				$this->success('当前邮箱删除成功！');
			}
			...
			$where['id']=array('in',$ids);
			$rs=$this->bll->where($where)->save($m);
			if($rs === false) $this->error('当前邮箱'.$message.'失败，请刷新网络重试！');
			else{
				$this->addLog('pass',$message.'邮箱IDS：'.$ids);
				$this->success('当前邮箱改变审核状态成功！');
			}
	...
	public function edit(){
		...
		if(IS_POST){
			...
			if($pid){
				...
				if(empty($info)) $this->error('当前邮箱不存在或被删除');
				$rs=$this->bll->where(array('id'=>$pid))->save($m);
				if($rs == false) $this->error('当前邮箱修改失败，请刷新重试！');
				else{
					$this->addLog('update','修改邮箱ID：'.$pid);
					$this->success('修改成功');
				}
			}else{
				...
				$m['addtime']=time();
				$sid=$this->bll->add($m);
				if($sid === false) $this->error('当前邮箱添加失败，请刷新重试！');
				else{
					$this->addLog('add','添加邮箱ID：'.$id);
					$this->success('添加邮箱成功');
				}
			}
		}
