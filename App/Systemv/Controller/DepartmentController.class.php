...
class DepartmentController extends BaseController{
	...
	public function operate(){
		...
		if($event == 'delete'){
			...
			if($rs === false) $this->error('当前科室删除失败，请刷新网络重试！');
			else{
				$this->addLog('delete','删除科室IDS：'.$ids);
				$this->success('当前科室删除成功！');
			}
			...
			$where['id']=array('in',$ids);
			$rs=$this->bll->where($where)->save($m);
			if($rs === false) $this->error('当前科室'.$message.'失败，请刷新网络重试！');
			else{
				$this->addLog('pass',$message.'科室IDS：'.$ids);
				$this->success('当前科室改变审核状态成功！');
			}
		}else if($event == 'sort'){
	...
	public function edit(){
		...
		if(IS_POST){
			...
			if($pid){
				...
				if(empty($info)) $this->error('当前科室不存在或被删除');
				$rs=$this->bll->where(array('id'=>$pid))->save($m);
				if($rs == false) $this->error('当前科室修改失败，请刷新重试！');
				else{
					$this->addLog('update','修改科室ID：'.$pid);
					$this->success('修改成功');
				}
			}else{
				...
				$m['addtime']=time();
				$sid=$this->bll->add($m);
				if($sid === false) $this->error('当前科室添加失败，请刷新重试！');
				else{
					$this->addLog('add','添加科室ID：'.$id);
					$this->success('添加科室成功');
				}
			}
		}
