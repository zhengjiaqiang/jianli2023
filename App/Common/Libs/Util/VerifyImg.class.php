...
class VerifyImg{
    ...
    public function entry($id = '') {
        ...
        // 验证码字体随机颜色
        $this->_color = imagecolorallocate($this->_image, mt_rand(1,170), mt_rand(1,170), mt_rand(1,170));
        // 验证码使用随机字体
        $ttfPath = CORE_PATH . '/Verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        if($this->useZh){ // 中文验证码
            ...
            for ($i = 0; $i<$this->length; $i++) {
                ...
                //$codeNX  += mt_rand($this->fontSize*0.5, $this->fontSize*1);
								//if($i >0){$codeNX+=$this->fontSize*0.4;}
								$codeNX = $xs * $i + mt_rand(1, 5);
								if($i >0){$codeNX = $xs * $i + mt_rand(1, 5);}
								else{$codeNX = mt_rand(3, 8);}
                //imagettftext($this->_image, $this->fontSize, mt_rand(-30, 30), $codeNX, $this->imageH / 1.5, $this->_color, $this->fontttf, $code[$i]);
		imagestring($this->_image,5,$codeNX,$this->imageH / 2.5,$code[$i],$this->_color);
            }
        }
    ...
    private function _writeNoise() {
        ...
        for ($i = 0; $i < 100; $i++) {
            $color = imagecolorallocate($this->_image, mt_rand(180, 255), mt_rand(180, 255), mt_rand(180, 255));
            imagestring($this->_image, mt_rand(1, 5), mt_rand(0, $this->imageW), mt_rand(0, $this->imageH), $codeSet[mt_rand(0, 29)], $color);
        }
    }
