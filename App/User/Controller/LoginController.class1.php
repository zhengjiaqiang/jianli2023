...
class LoginController extends Controller {
    ...
    public function ulogin(){
        ...
        // $verify = new \Common\Libs\Util\VerifyImg();
		// if(!$verify->check($code, 330))  $this->error('当前验证码错误！');
    ...
    public function reset(){ 
        $this->assign('type',C('SYS_SET.FORGETPASSWORD'));
        $this->display();
    ...
    public function sendreset(){
        ...
        $uinfo=M('user')->field('rnd,id')->where(C('SYS_SET.FORGETPASSWORD') == 1 ?  ['mobile'=>$phone] : ['email'=>$phone])->find();
        if(C('SYS_SET.FORGETPASSWORD') == 1){
        ...
        }elseif(C('SYS_SET.FORGETPASSWORD') == 2){
    ...
    public function sendsms(){
        ...
        webservice(C('SYS_SET.SENDMESSAGEWEB'),['username'=>C('SENDMESSAGEUSER'),'password'=>C('SENDMESSAGEPASSWD'),'destAddr'=>$mobile,'content'=>str_replace('{$code}',$code,C('SENDMESSAGEMESSAGE'))]);
        $data['uid']=$uinfo['id'];
        $data['code']=$code;
        $data['addtime']=time();
    ...
    public function resetpwd(){
        if(IS_POST){
            if(C('SYS_SET.FORGETPASSWORD') == 1 && !session('?istrue')) $this->error('未发送短信，前往短信发送...','reset');
            else if(C('SYS_SET.FORGETPASSWORD') == 2){
                if(!session('?send_email_time')) $this->error('未发送邮箱,前往发送邮箱');
            ...
            $result='';
            if(C('SYS_SET.FORGETPASSWORD') == 1) $result=M('user')->where(['id'=>session('istrue')])->save($m);
            else if(C('SYS_SET.FORGETPASSWORD') == 2) $result=M('user')->where(['email'=>session('send_email_user')])->save($m);
            if($result == false) $this->error('修改密码失败，请重新输入！');
            else $this->success('修改密码成功，前往登录...','/icenter');
        }
        //判断当前模式
        if(C('SYS_SET.FORGETPASSWORD') == 1) session('?istrue') ?: redirect('reset');
        else if(C('SYS_SET.FORGETPASSWORD') == 2){
