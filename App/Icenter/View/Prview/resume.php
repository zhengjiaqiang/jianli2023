			  	...
			  	<tbody>
				  	<tr>
				    	<td align="center" valign="center">
							...
							<input type="radio" name="status"   title="入库操作" lay-verify="req" value="8"></button><button type="button" class="btn btn-purple btn-sm" lay-submit lay-filter="setresume">确定</button> -->
							<button type="button" class="btn btn-purple btn-sm" data-status='10'  lay-submit lay-filter="setresume">需要</button>
							<button type="button" class="btn btn-purple btn-sm" data-status='8' lay-submit lay-filter="setresume">不需要</button>
...
layui.use(['jquery','layer','form'],function(){
	...
	form.on('submit(setresume)', function(data){
		data.field.status=$(this).data('status');
		$.post('/prview/isnext',data.field,function(_res){
			var index=layer.msg(_res.info,{icon:(_res.status== 0 ? 2 : 1),time:2000},function(){
