...
<style type="text/css">
	...
	.layui-card-header{background: #eee;border-left: 2px solid #F60;}
	.step-parent{position: fixed;bottom:30px;text-align: center;width: 860px;background: #fff;}
	.layui-form-radioed>i{color: #5FB878 !important;}
	.jianli .tab_bot{margin-bottom:50px;}
	.layui-form-select dl{right:0;left:auto;}
</style>
...
<script> 
	layui.config({
		...
		var $=layui.jquery,form=layui.form,layer=layui.layer,laydate=layui.laydate,index,upload=layui.upload,imgMax={$max};
		form.verify({
		...
		var fun={
			...
			_list:function(){
				...
				$.getJSON('/icenter/resume/form',{'id':_this._data.id},function(data){
					if(data.code !=0) { layer.msg('当前实例化失败，请刷新重试',{icon:2,time:2000}); return false;}
					var titleHtml='';
					...
					$.each(data.user,function(k,v){
						if($("[name='"+k+"']").is('select')) {
							...
							switch(type){
								case 'text':$("[name='"+k+"']").val(v); break;
								case 'number':$("[name='"+k+"']").val(v); break;
								case 'radio': 
								case 'checkbox':$("[name='"+k+"'][value='"+v+"']").prop("checked",true); break;
							}
						}
	...
	                }
			...
			_save:function(){
				...
				form.on('submit(save)',function(data){
					...
					$.post('/icenter/resume/usersave',{"value":_this._serializeToKeyValue(),"select":_this._serializeSelectToKeyValue(),'title':_this._serializeTitleToKeyValue()} ,function(res){
						...
						if(!res.status) layer.msg(res.message,{icon:2,time:2000});
							...
							if(_this._data.id <_this._data.maxid){
								_this._data.id++;
								_this._list();
							}
							else{
			...
			_prevstep:function(){
				...
				$('.prevstep').click(function(){
					if(_this._data.id >_this._data.minid){
						_this._data.id--;
						_this._list();
					}
				})
			...
			_listRender:function(){	
				var _this=this,html='<button name="'+_this._generateMixed(16)+'" class="magicalcoder-layupload layui-btn" type="button" mc-attr-str-url="/icenter/Resume/layuiUpload">上传文件</button>';
			...
			_xueli:function(){
				...
				form6.prepend("<option value=''>请选择专业</option>");
               	...
               	$('#form2').attr('data-value');$('#form3').attr('data-value')
