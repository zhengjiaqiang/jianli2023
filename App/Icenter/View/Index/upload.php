...
.myjianli{
	...
	<div id="content">
	    ...
	    <div id="right">
	    	<div class="title"><h3>附件上传<span>（只能上传图片格式，jpg、png；请控制附件质量在5M以内）</span></h3></div>
	        <div class="download">
				...
				<foreach name="images" item="v" key="k">
					...
					<div class="resetdiv1">
                        ...
                        <if condition="($k eq 7)">
                            <img src="/static/letter/letter.png" alt="" style="width: 80%;margin: 0 10%">
                        </if>
						...
						<div class="resettable2">
										...
										<foreach name="filelist[$k]" item="value">
											<li>
                                                <a target="_blank" href="{$value['filepath']}">
                                                    <if condition="$k eq 6 and $value.ext eq pdf">
                                                        <div  <if condition="$k eq 6 and $value.ext eq pdf">class="myjianli" </if> > <div>{$value['filename']} <span class="remove" data-id="{$value['id']}">✖</span></div></div>
                                                        <else/>
                                                         <img src="{$value['filepath']}"/>
                                                    </if>
												</a>
                                                <if condition="($k eq 6) and ($value.ext neq pdf)">
                            ...
                            <if condition="$k eq 6">
                                <button class="download-btn" lay-data="{parentid:{$k}}">✚&nbsp;上传文件</button>
                                  <else/>
